...
class BaseVM(object):
    ...
    ipv6 = True
    def __init__(self, debug=False, vcpus=None, config={}):
        self._guest = None
        # Start with the default config.  Allow input config to override defaults.
        ...
        self._config['ssh_key'] = open(self._config['ssh_key_file']).read()
        self._config['ssh_pub_key'] = open(self._config['ssh_pub_key_file']).read()
        ...
        self._args = [ \
            ...
            "-netdev", "user,id=vnet,hostfwd=:127.0.0.1:{}-:22".format(self._config['ssh_port']) +
    ...
    def wait_ssh(self, seconds=600):
        ...
        while datetime.datetime.now() < endtime:
            if self.ssh("exit 0") == 0 and self.ssh_root("exit 0") == 0:
                guest_up = True
                break
...
def parse_config(config, args):
    # Parse yaml config
    # and populate our config structure.
    if args.config:
    ...
    if 'target-conf' in yaml_dict:
        ...
        if any (k in target_dict for k in ("ssh_key","ssh_pub_key")) and \
           not all (k in target_dict for k in ("ssh_key","ssh_pub_key")):
            missing_key = "ssh_pub_key" if 'ssh_key' in target_dict else "ssh_key"
        ...
        if 'qemu_args' in target_dict:
            qemu_args = target_dict['qemu_args'].replace('\n', ' ').replace('\r', '')
...
def parse_args(vmcls):
    ...
    parser.add_option("--config", "-c", default=None,
                      help="Provide config yaml for configuration. See config_example.yaml for example.")
    parser.disable_interspersed_args()
    return parser.parse_args()

def main(vmcls, config=DEFAULT_CONFIG):
    try:

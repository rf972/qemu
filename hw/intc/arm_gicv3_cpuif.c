...
static const ARMCPRegInfo gicv3_cpuif_reginfo[] = {
    ...
    { .name = "ICC_SGI1R",
      .cp = 15, .opc1 = 0, .crm = 12,
      .type = ARM_CP_64BIT | ARM_CP_GIC | ARM_CP_NO_RAW,
      .access = PL1_W, .accessfn = gicv3_sgi_access,
      .writefn = icc_sgi1r_write,
    },
...
void arm_cpu_gic_lock_impl(CPUState *cpu, const char *file, int line)
{
    ...
    CPUARMState *env = &arm_cpu->env;
    GICv3CPUState *cs = icc_cs_from_env(env);
    arm_gic_lock_impl(cs->gic, file, line);
}
void arm_cpu_gic_unlock_impl(CPUState *cpu, const char *file, int line)
{
    ...
    CPUARMState *env = &arm_cpu->env;
    GICv3CPUState *cs = icc_cs_from_env(env);
    arm_gic_unlock_impl(cs->gic, file, line);
}
